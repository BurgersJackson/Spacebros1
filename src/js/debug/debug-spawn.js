...
export function spawnCruiser() {
  ...
  if (deps.destroyer) {
    ...
    if (
      ...
      typeof deps.destroyer.pixiCleanupObject === "function"
    ) {
      deps.destroyer.pixiCleanupObject();
    }
    deps.destroyer = null;
...
export function spawnStation() {
  ...
  console.log(
    "[DEBUG] Station spawned at",
    GameContext.spaceStation.pos.x.toFixed(0),
    GameContext.spaceStation.pos.y.toFixed(0)
  );
}
...
export function spawnDungeonBoss(bossType) {
  ...
  try {
    ...
    if (musicEnabled) setMusicMode("cruiser");
    console.log(
      `[DEBUG] ${bossType} spawned successfully at`,
      GameContext.boss.pos.x.toFixed(0),
      GameContext.boss.pos.y.toFixed(0)
    );
  } catch (err) {
...
export function spawnNecroticHive() {
  spawnDungeonBoss("NecroticHive");
}
export function spawnCerebralPsion() {
  spawnDungeonBoss("CerebralPsion");
}
export function spawnFleshforge() {
  spawnDungeonBoss("Fleshforge");
}
export function spawnVortexMatriarch() {
  spawnDungeonBoss("VortexMatriarch");
}
export function spawnChitinusPrime() {
  spawnDungeonBoss("ChitinusPrime");
}
export function spawnPsyLich() {
  spawnDungeonBoss("PsyLich");
}
...
export function initDebugKeyboardShortcuts() {
  ...
  document.addEventListener("keydown", function (e) {
    if (e.ctrlKey && e.shiftKey && (e.key === "e" || e.key === "E")) {
      ...
      showOverlayMessage(
        `HITBOX DEBUG: ${GameContext.DEBUG_COLLISION ? "ON" : "OFF"}`,
        GameContext.DEBUG_COLLISION ? "#0f0" : "#f00",
        1500
      );
      ...
      showOverlayMessage(
        `SHIELD BYPASS DEBUG: ${GameContext.DEBUG_SHIELD_BYPASS ? "ON" : "OFF"}`,
