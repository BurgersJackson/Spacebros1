...
export function checkDespawn(entity, range = 6000) {
  ...
  const dist = Math.hypot(
    entity.pos.x - GameContext.player.pos.x,
    entity.pos.y - GameContext.player.pos.y
  );
  if (dist > range) {
    entity.dead = true;
  }
...
export function applyAOEDamageToPlayer(aoeX, aoeY, aoeRadius, totalDamage, bypassShields = false) {
  ...
  const playerAngleToAOE = Math.atan2(
    aoeY - GameContext.player.pos.y,
    aoeX - GameContext.player.pos.x
  );
