...
export function killPlayer() {
    ...
    setTimeout(() => {
        ...
        deps.resetWarpState();
        deps.stopMusic();
        try { deps.depositMetaNuggets(); } catch (e) { console.warn('meta deposit failed', e); }
        if (GameContext.currentProfileName) {
            deps.autoSaveToCurrentProfile();
            deps.saveMetaProfile();
        }
...
export function togglePause() {
    ...
    if (deps.getFromPauseMenu && deps.getFromPauseMenu() && GameContext.gamePaused && document.getElementById('start-screen').style.display === 'block') {
        document.getElementById('start-screen').style.display = 'none';
