...
function handleMenuNavigation(now) {
    if ((!GameContext.gameActive || GameContext.gamePaused) && now - menuDebounce > 150) {
        ...
        if (activeElements.length > 0) {
            const menuChanged = !GameContext.gpState.lastMenuElements ||
            ...
            const leftPressed = GameContext.gpState.lastPadAxes?.x < -0.5 || (GameContext.gpState.lastPadButtons?.left);
            const rightPressed = GameContext.gpState.lastPadAxes?.x > 0.5 || (GameContext.gpState.lastPadButtons?.right);
            ...
            if (isSlider) {
                if (leftPressed || rightPressed) {
                    ...
                    let change = 0;
                    if (GameContext.gpState.lastPadAxes?.y < -0.5 || GameContext.gpState.lastPadButtons?.up) change = -1;
                    if (GameContext.gpState.lastPadAxes?.y > 0.5 || GameContext.gpState.lastPadButtons?.down) change = 1;
                ...
                let vertChange = 0;
                if (GameContext.gpState.lastPadAxes?.y < -0.5 || GameContext.gpState.lastPadButtons?.up) vertChange = -1;
                if (GameContext.gpState.lastPadAxes?.y > 0.5 || GameContext.gpState.lastPadButtons?.down) vertChange = 1;
...
export function initInputListeners() {
    ...
    window.addEventListener('keydown', e => {
        ...
        if (e.key === 'Escape' && _togglePause) _togglePause();
        if (e.key === 'F1') {
            e.preventDefault();
        ...
        if (e.ctrlKey && e.shiftKey && (e.code === 'Digit2' || e.code === 'Numpad2')) {
            ...
            const doJump = () => {
                ...
                if (_showOverlayMessage) _showOverlayMessage("DEBUG: ENTERING SECTOR 2 CAVE", '#0ff', 1200, 5);
    ...
    window.addEventListener('mousemove', e => {
        ...
        if (dx + dy >= 10 || document.pointerLockElement === _canvas) GameContext.lastMouseInputAt = now;
