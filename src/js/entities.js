...
import { Vector, playSound, colorToPixi } from './utils.js';
import { allocPixiSprite, releasePixiSprite, pixiCleanupObject } from './pixi-utils.js';
...
export class Coin extends Entity {
    ...
    draw(ctx) {
        if (this.dead) {
            if (this.sprite) releasePixiSprite(state.pools.pickup, this.sprite);
            this.sprite = null;
            return;
        }
        ctx.save();
    ...
    update() {
        ...
        if (!persistentContractWall) {
            ...
            if (player) {
                ...
                if (dist > 5000) this.dead = true;
            }
        }
    }

    break() {
        if (this.dead || this.unbreakable) return;
        this.dead = true;
        if (this.sizeLevel > 1) {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                if (state.entities && state.entities.asteroids) state.entities.asteroids.push(a);
            }
        }
    }
...
export class GateKey extends Entity {
    ...
    draw(ctx) {
        ...
        ctx.beginPath();
        ctx.moveTo(8, 0); ctx.lineTo(22, 0); ctx.lineTo(22, 6); ctx.lineTo(18, 6); ctx.lineTo(18, 2); ctx.lineTo(8, 2);
...
export class FlagshipWarpZone extends Entity {
    ...
    update() {
        if (this.dead || !state.player || state.player.dead) return;
        this.t++;
...
export class SectorPOI extends Entity {
    ...
    draw(ctx) {
        ...
        ctx.beginPath(); ctx.moveTo(0, -18); ctx.lineTo(16, 0); ctx.lineTo(0, 18); ctx.lineTo(-16, 0); ctx.closePath();
        ctx.fill(); ctx.stroke();
...
export class ExplorationCache extends Entity {
    ...
    draw(ctx) {
        ...
        ctx.scale(scale, scale); ctx.rotate(this.flash * 0.05);
        const grad = ctx.createLinearGradient(-12, -12, 12, 12);
        grad.addColorStop(0, '#ff0'); grad.addColorStop(0.5, '#ffa500'); grad.addColorStop(1, '#0ff');
