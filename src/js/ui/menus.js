...
export function showRunUpgrades() {
  ...
  const collectedUpgrades = Object.entries(GameContextRef.player.inventory).filter(
    ([id, tier]) => tier > 0
  );
  ...
  if (collectedUpgrades.length === 0) {
    ...
    UPGRADE_DATA.categories.forEach(category => {
      const categoryUpgrades = category.upgrades.filter(
        u => GameContextRef.player.inventory[u.id] > 0
      );
...
function grantDebugUpgrade(upgradeId, tier, upgradeName) {
  ...
  const action =
    tier > prevTier
      ...
      : tier < prevTier
        ? `DOWNGRADED to Tier ${tier}`
        : `RESET to Tier ${tier}`;
  if (showOverlayMessageRef)
    showOverlayMessageRef(`[DEBUG] ${upgradeName}: ${action}`, "#ff0", 1500, 10);
...
export function updateShipSelectionUI() {
  ...
  if (selectedShipType === "slacker") {
    ...
    descEl.textContent =
      "Mouse-driven • Click & hold LEFT BUTTON to brake • RIGHT BUTTON activates turbo boost\nShip follows your cursor • Keyboard works too • Perfect for tactical positioning";
  } else {
...
let gamepadFocusedShipBtn = null;

function setGamepadShipFocus(btnId) {
  gamepadFocusedShipBtn = btnId;
}

function confirmGamepadShipSelection() {
  if (!gamepadFocusedShipBtn) return;
  const shipType = gamepadFocusedShipBtn === "ship-slacker-btn" ? "slacker" : "standard";
  if (setSelectedShipTypeRef && getSelectedShipTypeRef && getSelectedShipTypeRef() !== shipType) {
    setSelectedShipTypeRef(shipType);
    updateShipSelectionUI();
  }
}
...
export function initMenuUi() {
  if (typeof window !== "undefined") {
    window.setGamepadShipFocus = setGamepadShipFocus;
    window.confirmGamepadShipSelection = confirmGamepadShipSelection;
  }
  updateShipSelectionUI();
  ...
  if (resumeStartBtn) {
    resumeStartBtn.addEventListener("click", () => {
      ...
      if (
        GameContextRef &&
        GameContextRef.gamePaused &&
        GameContextRef.pauseStartTime &&
        getGameNowMsRef &&
        shiftPausedTimersRef
      ) {
  ...
  if (resumeBtn)
    resumeBtn.addEventListener("click", () => {
      if (togglePauseRef) togglePauseRef();
    });
