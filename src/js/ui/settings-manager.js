...
export function updateStartScreenDisplay() {
  ...
  if (el) {
    el.innerText = GameContextRef.currentProfileName
      ? `Current: ${GameContextRef.currentProfileName}`
      : "Current: None";
  }
}
...
export function showSaveMenu() {
  ...
  if (profiles.length > 0) {
    const currentProfileExists =
      GameContextRef.currentProfileName &&
      profiles.some(p => p.name === GameContextRef.currentProfileName);
    selectedProfileName = currentProfileExists
      ? GameContextRef.currentProfileName
      : profiles[0].name;
  } else {
  ...
  if (profiles.length === 0) {
    listEl.innerHTML =
      '<div style="color: #888; text-align: center; padding: 20px;">No profiles found</div>';
  } else {
    profiles.forEach(p => {
...
export function initSettingsMenu() {
  ...
  if (settingsApplyBtn && isElectron) {
    settingsApplyBtn.addEventListener("click", async () => {
      ...
      const oldRes = old.internalResolution || {
        width: old.width || 1920,
        height: old.height || 1080
      };
      ...
      if (resolutionChanged || fullscreenChanged) {
        ...
        setTimeout(async () => {
          ...
          if (
            window.SpacebrosApp &&
            window.SpacebrosApp.settings &&
            window.SpacebrosApp.settings.isFullscreen
          ) {
            try {
