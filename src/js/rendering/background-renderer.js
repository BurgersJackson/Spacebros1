      ...
      if (ENABLE_NEBULA && pixiNebulaLayer) {
        ...
        const makeNebulaTileTexture = (tileSize, blobCount, paletteHex) => {
          ...
          for (let i = 0; i < blobCount; i++) {
            ...
            const r = tileSize * (0.2 + Math.random() * 0.55);
          ...
          const vign = cctx.createRadialGradient(
            tileSize * 0.5,
            tileSize * 0.5,
            tileSize * 0.1,
            tileSize * 0.5,
        ...
        const idxBase =
          typeof GameContext.sectorIndex === "number" && isFinite(GameContext.sectorIndex)
            ? Math.abs(GameContext.sectorIndex) | 0
            : 0;
        const paletteIdx = idxBase % palettes.length;
        const palette = palettes[paletteIdx];

        const nebulaLayers = [
          { tileSize: 1024, blobs: 9, alphaMult: 1.0, parallax: 0.01 },
          { tileSize: 768, blobs: 7, alphaMult: 0.66, parallax: 0.018 }
        ];

        const needsNebulaRebuild =
          pixiNebulaPaletteIdx !== paletteIdx || pixiNebulaTiles.length === 0;
        if (needsNebulaRebuild) {
          ...
          for (const L of nebulaLayers) {
            ...
            const spr = new PIXI.TilingSprite(
              tex,
              typeof width === "number" && width > 0 ? width : 1920,
              typeof height === "number" && height > 0 ? height : 1080
            );
            const mult = typeof L.alphaMult === "number" && isFinite(L.alphaMult) ? L.alphaMult : 1;
            const baseAlpha =
              typeof NEBULA_ALPHA === "number" && isFinite(NEBULA_ALPHA) ? NEBULA_ALPHA : 0.12;
            // Reduce alpha when CRT/bloom filters are active to prevent over-brightening
            const crtEnabled =
              typeof isCrtFilterEnabled === "function" ? isCrtFilterEnabled() : false;
      ...
      if (pixiStarTilingLayer) {
        ...
        if (pixiStarTiles.length === 0) {
          ...
          const layers = [
            ...
            { count: 70, minSize: 2, maxSize: 3, minAlpha: 0.08, maxAlpha: 0.4, parallax: 0.12 }
          ];
          for (const L of layers) {
            const tex = makeStarTileTexture(
              tileSize,
              L.count,
              L.minSize,
              L.maxSize,
...
export function updatePixiBackground(
  camX,
  camY,
  viewportWidth,
  viewportHeight,
  ...
  const isVerticalScrolling =
    typeof GameContext !== "undefined" && GameContext.verticalScrollingMode;
  ...
  if (
    ENABLE_NEBULA &&
    actualPixiNebulaLayer &&
    (!pixiNebulaTiles || pixiNebulaTiles.length === 0) &&
    actualPixiScreenRoot &&
    actualPixiApp &&
    actualPixiApp.renderer
  ) {
