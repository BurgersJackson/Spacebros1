...
const MEDKIT_URL = "assets/medkit.png";
...
export function initTextureAssets() {
  ...
  medkitImage.addEventListener("load", () => {
    medkitLoaded = true;
    applyHealthTexture();
  });
  medkitImage.addEventListener("error", () => {
    medkitLoaded = false;
  });
  medkitImage.src = MEDKIT_URL;
  ...
  asteroidImages = [new Image(), new Image(), new Image()];
  ...
  // NOTE: nuggetImage and medkitImage event listeners are now attached earlier
  // (before setting src) to handle cached images that fire load events synchronously
...
/**
 * Apply loaded medkit image to pixiTextures.health (used by HealthPowerUp).
 * @returns {void}
 */
export function applyHealthTexture() {
  if (!medkitLoaded || medkitTexture || !window.PIXI || !pixiTextures) return;
  try {
    const tex = PIXI.Texture.from(medkitImage);
    try {
      tex.baseTexture.scaleMode = PIXI.SCALE_MODES.LINEAR;
    } catch (e) {}
    try {
    ...
    medkitTexture = tex;
    pixiTextures.health = tex;
    // Bypassing potential alias issues
    pixiTextures.health_fixed = tex;
    console.log("[TextureManager] Health texture applied:", tex);
    console.log("[TextureManager] Medkit Image Src:", medkitImage.src);
  } catch (e) {
    console.error("[TextureManager] Failed to apply health texture:", e);
  }
}
