...
export function drawMinimap(pixiMinimapGraphics, canvas) {
  ...
  const refX = warpActive
    ...
    : GameContext.player
      ...
      : 0;
  const refY = warpActive
    ? GameContext.warpZone.pos.y
    : GameContext.player
  ...
  if (warpActive && GameContext.warpZone) {
    const segs =
      typeof GameContext.warpZone.allSegments === "function"
        ? GameContext.warpZone.allSegments()
        : [];
  ...
  if (
    ...
    !GameContext.radiationStorm.dead &&
    GameContext.player
  ) {
    ...
    if (dist * scale > 95) {
      ...
      pixiMinimapGraphics.drawCircle(
        centerX + dx * scale,
        centerY + dy * scale,
        Math.max(4, GameContext.radiationStorm.radius * scale)
      );
    }
  }

  // Coins (yellow) on minimap - draw as small circles for visibility
  pixiMinimapGraphics.beginFill(0xffff00);
  GameContext.coins.forEach(c => {
    if (GameContext.player) {
      ...
      if (inBounds(dx, dy)) {
        pixiMinimapGraphics.drawCircle(centerX + dx, centerY + dy, 2);
      }
    }
  });
  pixiMinimapGraphics.endFill();

  // Gold nuggets (orange) on minimap
  pixiMinimapGraphics.beginFill(0xff8800);
  GameContext.goldNuggets.forEach(gn => {
    if (GameContext.player) {
      const dx = (gn.pos.x - GameContext.player.pos.x) * scale;
      const dy = (gn.pos.y - GameContext.player.pos.y) * scale;
      if (inBounds(dx, dy)) {
        pixiMinimapGraphics.drawCircle(centerX + dx, centerY + dy, 2);
      }
    }
  ...
  // Magnet pickups (cyan) on minimap
  if (GameContext.player && GameContext.magnetPickups && GameContext.magnetPickups.length > 0) {
    for (const m of GameContext.magnetPickups) {
      if (!m || m.dead) continue;
      const dx = m.pos.x - GameContext.player.pos.x;
      ...
      pixiMinimapGraphics.beginFill(0x00ffff);
      if (inRange) {
        pixiMinimapGraphics.drawCircle(centerX + px, centerY + py, 5);
      } else {
        drawMinimapArrow(pixiMinimapGraphics, centerX + px, centerY + py, angle, 10, 8);
      }
      pixiMinimapGraphics.endFill();
  ...
  // Nuke pickups (orange) on minimap
  if (GameContext.player && GameContext.nukePickups && GameContext.nukePickups.length > 0) {
    for (const n of GameContext.nukePickups) {
      if (!n || n.dead) continue;
      const dx = n.pos.x - GameContext.player.pos.x;
      ...
      if (inRange) {
        pixiMinimapGraphics.drawCircle(centerX + px, centerY + py, 6);
      } else {
        drawMinimapArrow(pixiMinimapGraphics, centerX + px, centerY + py, angle, 10, 8);
      }
      pixiMinimapGraphics.endFill();
  ...
  if (!warpActive && GameContext.player && GameContext.miniEvent && !GameContext.miniEvent.dead) {
    let tx = GameContext.miniEvent.pos.x,
      ty = GameContext.miniEvent.pos.y;
    const dx = tx - GameContext.player.pos.x;
    const dy = ty - GameContext.player.pos.y;
    const dist = Math.hypot(dx, dy);
    const angle = Math.atan2(dy, dx);
...
export function initMinimapCanvas() {
  ...
  const minimapCtx = (() => {
    try {
      return minimapCanvas.getContext("2d", { desynchronized: true, alpha: false });
    } catch (e) {
      return minimapCanvas.getContext("2d");
    }
  })();
