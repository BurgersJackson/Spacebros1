...
export function initPixiOverlay(options) {
  ...
  const pixiStarLayer = new PIXI.ParticleContainer(8000, {
    position: true,
    tint: true,
    scale: true,
    alpha: true
  ...
  const pixiPickupLayer = new PIXI.ParticleContainer(6000, {
    position: true,
    rotation: true,
    tint: true,
    scale: true,
  ...
  const pixiParticleLayer = new PIXI.ParticleContainer(20000, {
    position: true,
    tint: true,
    scale: true,
    alpha: true
  ...
  const makeStarfieldTexture = (starCount, minSize, maxSize, minAlpha, maxAlpha) => {
    ...
    const tex = pixiApp.renderer.generateTexture(g, {
      region: new PIXI.Rectangle(0, 0, STAR_TILE_SIZE, STAR_TILE_SIZE)
    });
    try {
      tex.baseTexture.scaleMode = PIXI.SCALE_MODES.LINEAR;
  ...
  const makeNebulaTexture = (blobCount, palette) => {
    ...
    const tex = pixiApp.renderer.generateTexture(g, {
      region: new PIXI.Rectangle(0, 0, NEBULA_TILE_SIZE, NEBULA_TILE_SIZE)
    });
    try {
      tex.baseTexture.scaleMode = PIXI.SCALE_MODES.LINEAR;
      tex.baseTexture.wrapMode = PIXI.WRAP_MODES.REPEAT;
  ...
  try {
    ...
    const starTiles = [
      ...
      { sprite: new PIXI.TilingSprite(starTexNear, w, h), parallax: 0.1 }
    ];
...

  {
    ...
    const makeCruiser = () => {
      ...
      const podX = 16.5 * s;
      const podY = 10.0 * s;
      const hull = [L, 0, L - nose, H, -L + tail, H, -L, 0, -L + tail, -H, L - nose, -H];
