...
function setupCanvasResolution(internalW, internalH) {
  ...
  const caveGridSprite =
    typeof getPixiCaveGridSprite === "function" ? getPixiCaveGridSprite() : null;
  if (caveGridSprite) {
...
function handleWindowResize() {
  ...
  requestAnimationFrame(() => {
    ...
    const displayW =
      isElectronFullscreen || document.fullscreenElement ? window.innerWidth : internalW;
    const displayH =
      isElectronFullscreen || document.fullscreenElement ? window.innerHeight : internalH;
...
export function initCanvasSetup(options) {
  ...
  ctx = (() => {
    try {
      return canvas.getContext("2d", { desynchronized: true, alpha: false });
    } catch (e) {
  ...
  uiCtx = (() => {
    try {
      return uiCanvas.getContext("2d", { alpha: true });
    } catch (e) {
      return uiCanvas.getContext("2d");
    }
  })();
