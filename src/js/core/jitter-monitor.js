...
export class JitterMonitor {
  constructor() {
    this.enabled = false; // Disabled - enable via console if needed
    this.frameTimes = [];
  ...
  getRecommendations() {
    ...
    if (this.jitterCount > 10) {
      recommendations.push(
        "High jitter detected - Consider spreading entity cleanup across multiple frames"
      );
    }
    ...
    if (this.maxFrameTime > 50) {
      recommendations.push(
        "Large frame spikes detected - Check for blocking operations during gameplay"
      );
    }
