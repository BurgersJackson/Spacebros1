...
export class SuperFlagshipBoss extends Flagship {
  ...
  applyDamageToPlayer(amount) {
    ...
    if (remaining > 0) {
      ...
      if (_spawnParticles)
        _spawnParticles(GameContext.player.pos.x, GameContext.player.pos.y, 10, "#0ff");
    }
  ...
  update(deltaTime = SIM_STEP_MS) {
    ...
    if (GameContext.caveMode && GameContext.caveLevel && GameContext.caveLevel.active) {
      ...
      const cx = Math.max(
        bounds.left + pad,
        Math.min(bounds.right - pad, GameContext.caveLevel.centerXAt(this.pos.y))
      );
  ...
  kill() {
    ...
    // Spawn coins: 80 coins * 10 value = 800 total
    const coinCount = 80;
    const coinValue = 10;
    ...
    if (GameContext.gameStartTime) {
      ...
      if (GameContext.pauseStartTime)
        elapsed =
          GameContext.pauseStartTime - GameContext.gameStartTime - (GameContext.pausedAccumMs || 0);
