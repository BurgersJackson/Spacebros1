...
import { GameContext, getElapsedGameTime, getLevelHpScaling } from "../../core/game-context.js";
...
export class FinalBoss extends Entity {
  constructor(x, y, zone) {
    ...
    // 30px reduction from previous 830
  ...
  drawBossHud(ctx) {
    ...
    ctx.fillText(`FINAL BOSS ${Math.max(0, Math.floor(pct * 100))}%`, _canvas.width / 2, y - 2);
    ctx.restore();
  }
  ...
  update(deltaTime = SIM_STEP_MS) {
    ...
    if (now - this.lastShieldRegenAt >= this.shieldRegenMs) {
      ...
      if (Math.random() < 0.5 && _spawnParticles)
    ...
    if (this.phase >= 2 && !this.phase2Started) {
      ...
      showOverlayMessage("FINAL BOSS PHASE 2", "#f0f", 1800, 3);
    }
    if (this.phase === 3 && !this.phase3Started) {
      this.phase3Started = true;
      showOverlayMessage("FINAL BOSS PHASE 3", "#f0f", 1800, 3);
    }
    ...
    if (this.dashWarmup > 0) {
      ...
      const blend = 1 - Math.pow(0.8, dtFactor); // frame-independent lerp factor for 0.20
      ...
      showOverlayMessage("FINAL BOSS CHARGING", "#f0f", 700);
    }
    ...
    if (this.dashWarmup <= 0 && this.dashFrames <= 0) {
      ...
      const blend = 1 - Math.pow(0.9, dtFactor); // for 0.10 per frame
  ...
  maybeCallHelpers() {
    ...
    const hpPct = this.maxHp > 0 ? this.hp / this.maxHp : 0;
    const aliveHelpers = GameContext.enemies.filter(
  ...
  spawnHelpers(count) {
    ...
    showOverlayMessage("FINAL BOSS SUMMONED REINFORCEMENTS", "#f0f", 1200);
  }
  ...
  draw(ctx) {
    ...
    if (pixiBossLayer && pixiTextures && pixiTextures.final_boss) {
      ...
      if (!container) {
        ...
        const spr = new PIXI.Sprite(pixiTextures.final_boss);
        spr.anchor.set(0.5);
    ...
    for (let i = 0; i < count; i++) {
      ...
      mine.update = function (deltaTime = 16.67) {
        ...
        if (GameContext.player && !GameContext.player.dead) {
          ...
          if (dist < 100) {
            this.dead = true;
            if (_spawnFieryExplosion) _spawnFieryExplosion(this.pos.x, this.pos.y, 2.0);
            playSound("explosion");
            // Mine explosion damage (5 HP)
            if (_applyAOEDamageToPlayer) _applyAOEDamageToPlayer(this.pos.x, this.pos.y, 200, 5);
          }
        }
      };
