...
export class Cruiser extends Enemy {
  constructor(encounterIndex = 1) {
    ...
    this.hardpoints = [
      ...
      },
      {
        ...
        off: { x: 0, y: -12 * hs },
        r: 3.7 * hs,
        hp: Math.round(hardHp * 1.1),
        maxHp: Math.round(hardHp * 1.1),
        cd: 0
      },
      {
        id: "MB",
      ...
      },
      {
        ...
        off: { x: 0, y: 3 * hs },
        r: 3.4 * hs,
        hp: Math.round(hardHp * 1.2),
        maxHp: Math.round(hardHp * 1.2),
        cd: 0
      }
    ];
  ...
  kill() {
    ...
    if (this._pixiHardpointsGfx) {
      try {
        if (this._pixiHardpointsGfx.parent)
          this._pixiHardpointsGfx.parent.removeChild(this._pixiHardpointsGfx);
    ...
    if (this._pixiTelegraphGfx) {
      try {
        if (this._pixiTelegraphGfx.parent)
          this._pixiTelegraphGfx.parent.removeChild(this._pixiTelegraphGfx);
  ...
  update(deltaTime = SIM_STEP_MS) {
    ...
    if (this.phaseTimer <= 0) {
      ...
      for (let attempts = 0; attempts < this.phaseSeq.length; attempts++) {
        ...
        if ((next.name === "CURTAIN" || next.name === "SWEEP") && !this.hasHardpoint("sprayer"))
          continue;
  ...
  runPhaseAttacks() {
    ...
    if (this.phaseName === "SALVO") {
      if (this.phaseTick % 11 === 0) {
        this.fireCannons(aim, 2, 0.18, 18, this.cruiserBaseDamage, "#f44");
      }
    } else if (this.phaseName === "CURTAIN") {
      if (this.phaseTick % 5 === 0) {
        this.fireSprayerFan(aim, 8, 1.1, 10, 1, "#f0f");
      }
  ...
  fireCannons(baseAngle, shots, spread, speed, dmg, color) {
    ...
    for (let c = 0; c < cannons.length; c++) {
      ...
      for (let i = 0; i < shots; i++) {
        ...
        const shot = new Bullet(p.x, p.y, a, speed, {
          owner: "enemy",
          damage: dmg,
          radius: 4,
          color,
  ...
  drawBossHud(ctx) {
    ...
    const phaseText =
      this.invulnerableTimer > 0 ? `${this.phaseName} (INVULNERABLE)` : this.phaseName;
