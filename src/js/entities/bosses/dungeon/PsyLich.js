...
export class PsyLich extends Enemy {
    ...
    runPhaseAttacks() {
        ...
        if (this.phaseName === 'PHASE_IN') {
            ...
            if (this.phaseTick % 15 === 0) {
                ...
                if (_spawnParticles) _spawnParticles(this.pos.x + (Math.random() - 0.5) * 300, this.pos.y + (Math.random() - 0.5) * 300, 3, '#a0f');
            }
            ...
            if (this.phaseTick % 10 === 0) {
                // Rapid fire
                const b = new Bullet(this.pos.x, this.pos.y, aim, 12, { owner: 'enemy', damage: 1, radius: 3, color: '#f0a' });
                b.owner = this;
    ...
    drawBossHud(ctx) {
        ...
        const livesText = this.livesRemaining !== undefined ? ` (${this.livesRemaining + 1} LIVES)` : '';
