...
export class Shockwave extends Entity {
  ...
  update(deltaTime = SIM_STEP_MS) {
    ...
    if (GameContext.boss && GameContext.bossActive && !GameContext.boss.dead)
      targets.push(GameContext.boss);
    if (this.damagePlayer && GameContext.player && !GameContext.player.dead)
      targets.push(GameContext.player);
    ...
    for (let e of targets) {
      ...
      if (dist < this.currentRadius + e.radius) {
        if (e === GameContext.player) {
          if (GameContext.player.takeHit) GameContext.player.takeHit(this.damage); // Explosives bypass shields
          this.hitList.push(e);
        } else {
          ...
          if (
            !this.isEnemy &&
            GameContext.player.stats.critChance > 0 &&
            Math.random() < GameContext.player.stats.critChance
          ) {
          ...
          if (e.innerShieldSegments && e.innerShieldSegments.length > 0 && remaining > 0) {
            ...
            if (e.innerShieldRotation !== undefined && e.innerShieldRadius !== undefined) {
              for (let i = 0; i < e.innerShieldSegments.length; i++) {
                ...
                const shieldX =
                  e.pos.x + Math.cos(segAngle + e.innerShieldRotation) * e.innerShieldRadius;
                const shieldY =
                  e.pos.y + Math.sin(segAngle + e.innerShieldRotation) * e.innerShieldRadius;
          ...
          if (remaining > 0) {
            ...
            if (playSound) playSound("enemy_shield_hit");
          ...
          playSound("hit");
          if (_spawnParticles)
            _spawnParticles(e.pos.x, e.pos.y, 5, this.hasCrit ? "#ffd700" : "#ff0");
          ...
          if (!this.isEnemy && this.isExplosive) {
            ...
            for (let other of targets) {
              ...
              if (d < explodeRadius + other.radius) {
                ...
                if (other.shieldSegments && other.shieldSegments.length > 0) {
                  if (other.shieldRotation !== undefined && other.shieldRadius !== undefined) {
                    ...
                    for (let i = 0; i < other.shieldSegments.length && remaining > 0; i++) {
                      ...
                      const shieldX =
                        other.pos.x +
                        Math.cos(segAngle + other.shieldRotation) * other.shieldRadius;
                      const shieldY =
                        other.pos.y +
                        Math.sin(segAngle + other.shieldRotation) * other.shieldRadius;
                      const distToSegment = Math.hypot(this.pos.x - shieldX, this.pos.y - shieldY);
                ...
                if (
                  other.innerShieldSegments &&
                  other.innerShieldSegments.length > 0 &&
                  remaining > 0
                ) {
                  if (
                    other.innerShieldRotation !== undefined &&
                    other.innerShieldRadius !== undefined
                  ) {
                    ...
                    for (let i = 0; i < other.innerShieldSegments.length && remaining > 0; i++) {
                      ...
                      const shieldX =
                        other.pos.x +
                        Math.cos(segAngle + other.innerShieldRotation) * other.innerShieldRadius;
                      const shieldY =
                        other.pos.y +
                        Math.sin(segAngle + other.innerShieldRotation) * other.innerShieldRadius;
                      const distToSegment = Math.hypot(this.pos.x - shieldX, this.pos.y - shieldY);
                      if (distToSegment <= explodeRadius) {
          ...
          if (!this.isEnemy && GameContext.player.stats.comboMeter > 0) {
            GameContext.player.comboStacks = Math.min(
              GameContext.player.comboStacks + 1,
              GameContext.player.comboMaxStacks
            );
            ...
            if (!this.isEnemy && GameContext.player.stats.lifestealAmount > 0) {
              ...
              if (GameContext.player.hp < GameContext.player.maxHp) {
                GameContext.player.hp = Math.min(
                  ...
                  GameContext.player.hp + healAmount
                );
                if (_updateHealthUI) _updateHealthUI();
                if (_spawnParticles)
                  _spawnParticles(GameContext.player.pos.x, GameContext.player.pos.y, 5, "#0f0");
              }
            }
          }
        }
