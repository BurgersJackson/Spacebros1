...
export class WallTurret extends Entity {
  ...
  update(deltaTime = SIM_STEP_MS) {
    ...
    if (
      ...
      GameContext.activeContract.id !== this.contractId
    )
      return;
    ...
    GameContext.bullets.push(
      new Bullet(muzzleX, muzzleY, aim, 12, { owner: "enemy", damage: 1, radius: 4, color: "#f80" })
    );
    if (Math.random() < 0.25) {
      GameContext.bullets.push(
        new Bullet(muzzleX, muzzleY, aim + 0.12, 12, {
          owner: "enemy",
          damage: 1,
          radius: 4,
          color: "#f80"
