...
export class ShootingStar extends Entity {
  ...
  kill(dropNugz = false) {
    ...
    if (dropNugz) {
      ...
      const nuggetCount = Math.max(1, Math.floor(Math.random() * 7));
      const coinValue = 20 + Math.floor(Math.random() * 41);
      ...
      const c = new Coin(this.pos.x, this.pos.y, coinValue);
      c.vel.x = (Math.random() - 0.5) * 2;
      c.vel.y = (Math.random() - 0.5) * 2;
      GameContext.coins.push(c);
      playSound("coin");
    }
  }
