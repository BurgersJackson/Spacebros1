...
export class EnvironmentAsteroid extends Entity {
  ...
  update(deltaTime = SIM_STEP_MS) {
    ...
    if (
      ...
      (GameContext.verticalScrollingZone.state === "scrolling" ||
        GameContext.verticalScrollingZone.state === "boss_intro" ||
        GameContext.verticalScrollingZone.state === "boss_battle")
    ) {
  ...
  draw(ctx) {
    ...
    if (pixiAsteroidLayer && pixiTextures?.asteroids?.length > 0) {
      ...
      if (
        this.indestructible &&
        indestructibleTextureReady &&
        pixiTextures.asteroidIndestructible
      ) {
