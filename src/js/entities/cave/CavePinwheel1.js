/**
 * CavePinwheel1.js
 * Cave-specific pinwheel variant (standard type).
 * Similar to 'standard' Pinwheel: cyan/magenta shields, star burst fire pattern.
 */
 ...
 */
export function registerCavePinwheelDependencies(deps) {
...
export class CavePinwheel1 extends Entity {
    constructor(x, y) {
        super(0, 0);
        this.pos.x = x;
        this.pos.y = y;
    ...
    update(deltaTime = SIM_STEP_MS) {
        ...
        if (GameContext.player && !GameContext.player.dead) {
            ...
            let maxSpeed = 5.0; // standard speed
            maxSpeed *= speedRamp;
        ...
        if (GameContext.player && !GameContext.player.dead) {
            ...
            if (dist < fireRange) {
                ...
                if (this.shootTimer <= 0) {
                    ...
                    // Standard star burst pattern (3 bullets at 120°)
                    const damage = 2;
                    for (let i = 0; i < 3; i++) {
    ...
    draw(ctx) {
        ...
        if (pixiBaseLayer && pixiTextures) {
            ...
            // Shields (Graphics) - standard cyan/magenta colors
            if (pixiVectorLayer) {
                const shieldColor = 0x00ffff; // cyan
                const innerColor = 0xff00ff; // magenta
