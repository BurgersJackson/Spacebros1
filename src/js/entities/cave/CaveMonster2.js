...
export class CaveMonster2 extends CaveMonsterBase {
    ...
    hornArc(phase) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const b = new Bullet(this.pos.x, this.pos.y, a, 10, { damage: 11, life: 70, color: '#f00', isEnemy: true, radius: 10 });
    ...
    bellowShockwave(phase) {
        ...
        for (let i = 0; i < 30; i++) {
            ...
            // new Bullet(x, y, angle, speed, opts)
            const b = new Bullet(this.pos.x, this.pos.y, a, 5, { damage: 6, life: 400, color: '#f80', isEnemy: true, radius: 5 });
            GameContext.bullets.push(b);
        }
    }
    ...
    bloodRain(phase) {
        ...
        for (let i = 0; i < count; i++) {
            setTimeout(() => {
                ...
                // Falling bullet
                // new Bullet(x, y, angle, speed, opts)
                const b = new Bullet(x, y, Math.PI / 2, 12, { damage: 8, life: 60, color: '#f00', isEnemy: true, radius: 8 });
                GameContext.bullets.push(b);
            }, i * 50);
        }
        playSound('rapid_shoot');
