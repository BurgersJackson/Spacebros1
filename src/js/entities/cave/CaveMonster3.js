...
export class CaveMonster3 extends CaveMonsterBase {
  ...
  shieldDroneAttack(phase) {
    ...
    this.shieldDrone = {
      pos: {
        x: this.pos.x + Math.cos(droneAngle) * droneDist,
        y: this.pos.y + Math.sin(droneAngle) * droneDist
      },
      vel: { x: 0, y: 0 },
      angle: droneAngle,
      orbitDist: droneDist,
          ...
          if (dist < r + bullet.radius) {
            ...
            if (this.hp <= 0) {
              ...
              if (caveDeps.spawnParticles)
                caveDeps.spawnParticles(this.pos.x, this.pos.y, 30, "#80f");
              playSound("explosion");
            }
          }
        }
        ...
        if (GameContext.player && !GameContext.player.dead) {
          ...
          if (dist < 100) {
            ...
            if (
              ...
              if (caveDeps.applyAOEDamageToPlayer)
                caveDeps.applyAOEDamageToPlayer(this.pos.x, this.pos.y, explosionRadius, 15);
            }
          }
        }
      };
        ...
        if (GameContext.player && !GameContext.player.dead) {
          ...
          for (const beamAngle of this.beamAngles) {
            ...
            if (d <= hitDist) {
              ...
              if (caveDeps.applyAOEDamageToPlayer)
                caveDeps.applyAOEDamageToPlayer(
                  GameContext.player.pos.x,
                  GameContext.player.pos.y,
                  100,
                  15
  ...
  kill() {
    ...
    if (this._pixiBeamGfx) {
      try {
        this._pixiBeamGfx.destroy({ children: true });
      } catch (e) {}
