...
export class CaveGuidedMissile extends Entity {
  ...
  applyDamageToPlayer(amount) {
    ...
    if (GameContext.player.stats.secondWindActive > 0) {
      ...
      if (_spawnParticles)
        _spawnParticles(GameContext.player.pos.x, GameContext.player.pos.y, 5, "#80f");
      return;
    }
    ...
    if (GameContext.player.stats.evasion > 0 && Math.random() < GameContext.player.stats.evasion) {
      if (_spawnParticles)
        _spawnParticles(GameContext.player.pos.x, GameContext.player.pos.y, 8, "#0ff");
      playSound("shield_hit");
    ...
    if (remaining > 0) {
      ...
      if (GameContext.player.stats.thornArmor > 0 && this.hp) {
        ...
        if (
          this === GameContext.destroyer ||
          (this.displayName && this.displayName.includes("DESTROYER"))
        ) {
          const hpBefore = this.hp;
          this.hp -= reflectDamage;
          console.log(
            `[DESTROYER DEBUG] THORN ARMOR REFLECT: ${reflectDamage} damage | HP: ${hpBefore} -> ${this.hp}`
          );
        } else {
          this.hp -= reflectDamage;
        }
      ...
      if (GameContext.player.stats.secondWindFrames > 0) {
        ...
        if (_spawnParticles)
          _spawnParticles(GameContext.player.pos.x, GameContext.player.pos.y, 10, "#80f");
      }
  ...
  update(deltaTime = SIM_STEP_MS) {
    ...
    try {
      const nearby = GameContext.asteroidGrid
        ? GameContext.asteroidGrid.query(this.pos.x, this.pos.y)
        : [];
