...
export class CaveMonsterBase extends Entity {
  ...
  hitTestCircle(x, y, r) {
    ...
    if (dist <= this.innerShieldRadius) {
      ...
      const segmentIndex = Math.floor(
        (normalizedAngle / (Math.PI * 2)) * this.innerShieldSegments.length
      );
  ...
  applyDamageToPlayer(amount) {
    ...
    if (remaining > 0) {
      ...
      if (caveDeps.spawnParticles)
        caveDeps.spawnParticles(GameContext.player.pos.x, GameContext.player.pos.y, 14, "#f00");
      playSound("hit");
      ...
      if (caveDeps.spawnParticles)
        caveDeps.spawnParticles(GameContext.player.pos.x, GameContext.player.pos.y, 10, "#0ff");
    }
  ...
  kill() {
    ...
    if (caveDeps.showOverlayMessage)
      caveDeps.showOverlayMessage(`${this.displayName} DESTROYED`, "#0f0", 2800, 3);
    ...
    if (GameContext.bossesDestroyedCount >= 3) {
      ...
      if (caveDeps.showOverlayMessage) {
        caveDeps.showOverlayMessage("CAVE BOSS DEFEATED - SPACE STATION IN 30s", "#f80", 4000);
      }
    }
  }
  ...
  draw(ctx) {
    ...
    // Pixi rendering
    if (pixiEnemyLayer) {
      // Using pixiEnemyLayer for boss hull usually? or pixiBossLayer if it exists?
      ...
      const hullScale =
        this.visualRadius && isFinite(this.visualRadius) ? this.visualRadius / 170 : 1;
