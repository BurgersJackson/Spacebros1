...
export class CaveWallTurret extends Entity {
  ...
  update(deltaTime = SIM_STEP_MS) {
      ...
      if (this.trackerBurst > 0) {
        ...
        if (this.trackerBurstCd <= 0) {
          ...
          GameContext.bullets.push(
            new Bullet(muzzleX, muzzleY, a, 48, {
              owner: "enemy",
              damage: 1,
              radius: 4,
              color: "#0ff",
        ...
        if (this.trackerLock <= 0) {
          // Changed to <= 0 for safety with float decrement
    ...
    if (this.mode === "missile") {
      ...
      if (this.reload <= 0) {
        GameContext.guidedMissiles.push(
          new CaveGuidedMissile(this, {
            hp: 5,
            maxDamage: 5,
            radius: 36,
            speed: 8.2,
    ...
    if (this.mode === "beam") {
      const applyBeamDamageToPlayer = amount => {
        ...
        if (remaining > 0) {
          ...
          if (_spawnParticles)
            _spawnParticles(GameContext.player.pos.x, GameContext.player.pos.y, 8, "#ff0");
          playSound("shield_hit");
        }
    ...
    if (this.reload <= 0) {
      ...
      GameContext.bullets.push(
        new Bullet(muzzleX, muzzleY, aim, 42, {
          owner: "enemy",
          damage: 1,
          radius: 4,
          color: "#8ff",
      ...
      if (Math.random() < 0.25)
        GameContext.bullets.push(
          new Bullet(muzzleX, muzzleY, aim + 0.08, 42, {
            owner: "enemy",
            damage: 1,
            radius: 4,
            color: "#8ff",
