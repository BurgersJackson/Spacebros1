...
export class CaveLevel {
  ...
  generate() {
    ...
    this.entranceSeal = {
      y: sealY,
      segments: sealSegs,
      sideSegments: sideSegs,
      leftX: entranceLeftX,
      rightX: entranceRightX
    };
  ...
  resetFireWall(playerY = null) {
    const gap = 400;
    const baseY = typeof playerY === "number" && isFinite(playerY) ? playerY : this.startY + 600;
    this.fireWall = {
      y: baseY + gap,
  ...
  updateFireWall(deltaTime = 16.67) {
    ...
    const isBossFightActive =
      GameContext.bossActive && GameContext.boss && !GameContext.boss.dead && GameContext.caveMode;
  ...
  boundsAt(y) {
    ...
    const idx = Math.max(
      0,
      Math.min(this.leftPts.length - 1, Math.floor((this.startY - y) / this.stepY))
    );
  ...
  segmentsNearY(y, spanBuckets = 3) {
    ...
    const segs = [];
    for (
      let i = Math.max(0, idx - spanBuckets);
      i <= Math.min(this.buckets.length - 1, idx + spanBuckets);
      i++
    ) {
    ...
    if (this.entranceSeal && this.entranceSeal.segments && this.entranceSeal.segments.length) {
      if (Math.abs(this.entranceSeal.y - y) < this.stepY * (spanBuckets + 3))
        segs.push(...this.entranceSeal.segments);
    }
    if (
      this.entranceSeal &&
      this.entranceSeal.sideSegments &&
      this.entranceSeal.sideSegments.length
    ) {
      if (
        y > this.startY - this.stepY * (spanBuckets + 3) &&
        y < this.entranceSeal.y + this.stepY * (spanBuckets + 3)
      )
        segs.push(...this.entranceSeal.sideSegments);
    }
    if (
      !this.exitUnlocked &&
      this.exitSeal &&
      this.exitSeal.segments &&
      this.exitSeal.segments.length
    ) {
      if (Math.abs(this.exitSeal.y - y) < this.stepY * (spanBuckets + 3))
        segs.push(...this.exitSeal.segments);
    }
    if (
      !this.exitUnlocked &&
      this.exitSeal &&
      this.exitSeal.sideSegments &&
      this.exitSeal.sideSegments.length
    ) {
      if (
        y > this.exitSeal.y - this.stepY * (spanBuckets + 3) &&
        y < this.exitSeal.y + this.stepY * (spanBuckets + 6)
      )
        segs.push(...this.exitSeal.sideSegments);
    }
    ...
    for (let i = 0; i < this.doors.length; i++) {
      ...
      if (d && d.id === id) {
        ...
        if (caveDeps.showOverlayMessage)
          caveDeps.showOverlayMessage(d.open ? "DOOR OPENED" : "DOOR CLOSED", "#0ff", 900, 1);
        return;
      }
    }
  }
  ...
  onRelayDestroyed(gateIndex) {
    ...
    if (!gate || gate.open || !gate.relaysEnabled || !gate.relaysSpawned || gate.relaysCleared)
      return;
    gate.relaysRemaining = Math.max(0, (gate.relaysRemaining || 0) - 1);
    if (caveDeps.showOverlayMessage)
      caveDeps.showOverlayMessage(`RELAYS LEFT: ${gate.relaysRemaining}`, "#ff0", 800, 2);
    if (gate.relaysRemaining <= 0) {
      gate.relaysCleared = true;
      if (caveDeps.showOverlayMessage)
        caveDeps.showOverlayMessage("GATE SHIELD DOWN", "#0f0", 1600, 3);
      playSound("powerup");
      if (!gate.bossEnabled) {
  ...
  drawEntities(ctx, camX, camY, height, zoom) {
    ...
    let z = zoom || GameContext.currentZoom || ZOOM_LEVEL;
    if (!isFinite(z) || z <= 0) z = ZOOM_LEVEL;
    const safeCamX = isFinite(camX)
      ? camX
      : GameContext.player
        ? GameContext.player.pos.x - height / (2 * z)
        : 0;
    const safeCamY = isFinite(camY)
      ? camY
      : GameContext.player
        ? GameContext.player.pos.y - height / (2 * z)
        : 0;
  ...
  update(deltaTime = SIM_STEP_MS) {
    ...
    if (GameContext.player && !GameContext.player.dead && !this.finalSpawned) {
      ...
      if (
        ...
        !GameContext.caveBossArena.bossSpawned
      ) {
  ...
  spawnBoss(tier) {
    ...
    if (caveDeps.showOverlayMessage)
      caveDeps.showOverlayMessage("WARNING: TITAN CLASS DETECTED", "#f00", 3000, 5);
  ...
  checkArenaEntry() {
    if (!GameContext.player || !GameContext.caveBossArena || GameContext.caveBossArena.bossSpawned)
      return;
    ...
    if (pdist < GameContext.caveBossArena.radius) {
      // Enter arena radius
  ...
  updatePixi() {
    ...
    if (!GameContext.player || !this._pixiBackGfx || !this._pixiFrontGfx || !this._pixiBlockerGfx)
      return;
    ...
    const segFinite = s =>
      s && isFinite(s.x0) && isFinite(s.y0) && isFinite(s.x1) && isFinite(s.y1);
    ...
    if (
      this.fireWall &&
      this.fireWall.y < this.fireWall.minY + 20000 &&
      this.fireWall.y > y0 - 1000
    ) {
