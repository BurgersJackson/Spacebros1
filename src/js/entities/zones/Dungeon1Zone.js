...
export class Dungeon1Zone extends Entity {
  ...
  update(deltaTime = SIM_STEP_MS) {
    ...
    if (
      ...
      // Count dungeon boss kill toward quest progress
      if (!GameContext.caveMode) {
        GameContext.arenaFightsCompleted++;
        const target = GameContext.arenaFightTarget || 3;

        // Check if all bosses killed, start 3-minute countdown
        if (
          GameContext.arenaFightsCompleted >= target &&
          !GameContext.spaceStation &&
          !GameContext.stationSpawnAt
        ) {
          GameContext.stationSpawnAt = Date.now() + 180000; // 3 minutes
          showOverlayMessage(
            "ALL BOSSES DEFEATED - STATION INCOMING IN 3 MINUTES",
            "#f80",
            5000,
            2
          );
          ...
          showOverlayMessage(
            `BOSS KILLS: ${Math.min(GameContext.arenaFightsCompleted, target)}/${target}`,
            "#0f0",
            3000,
            2
          );
