...
export class Pinwheel extends Entity {
  ...
  update(deltaTime = SIM_STEP_MS) {
    ...
    if (GameContext.player && !GameContext.player.dead) {
      ...
      let px = GameContext.player.pos.x,
        py = GameContext.player.pos.y;
          ...
          if (this.type === "heavy") {
            for (let i = -1; i <= 1; i++) {
              ...
              GameContext.bullets.push(
                new Bullet(bx, by, a, 8, { owner: "enemy", damage: 3, life: 480, color: "#fa0" })
              );
            ...
            const by = this.pos.y + Math.sin(a) * 75;
            GameContext.bullets.push(
              new Bullet(bx, by, a, 14, { owner: "enemy", damage: 1, life: 180, color: "#0ff" })
            );
            if (_spawnBarrelSmoke) _spawnBarrelSmoke(bx, by, a);
            ...
            for (let i = 0; i < 3; i++) {
              ...
              GameContext.bullets.push(
                new Bullet(bx, by, a, 8, { owner: "enemy", damage: damage, life: 240 })
              );
              if (_spawnBarrelSmoke) _spawnBarrelSmoke(bx, by, a);
            }
            playSound("shoot");
            this.shootTimer = Math.round(
              (GameContext.difficultyTier >= 2 ? 40 : 75) * cooldownMult
            );
          }
        }
      }
  ...
  draw(ctx) {
    ...
    if (pixiBaseLayer && pixiTextures) {
      ...
      if (this._pixiTurretContainer) {
        try {
          this._pixiTurretContainer.destroy({ children: true });
        } catch (e) {}
        this._pixiTurretContainer = null;
      }
      this._pixiTurretBaseSpr = null;
