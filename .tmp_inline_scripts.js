        ...
        (function loadPhaser() {
            ...
            const cdnSrc = "https://cdn.jsdelivr.net/npm/phaser@3/dist/phaser.min.js";

            const script = document.createElement("script");
            script.src = localSrc;
            script.defer = true;
            script.onload = () => {
                if (window.Phaser && window.Phaser.VERSION) console.info(`[Phaser] Loaded v${window.Phaser.VERSION}`);
            };
            script.onerror = () => {
                ...
                fallback.defer = true;
                fallback.onload = () => {
                    if (window.Phaser && window.Phaser.VERSION) console.info(`[Phaser] Loaded v${window.Phaser.VERSION} (CDN)`);
                };
        ...
        function startMusic(mode = null) {
            ...
            if (musicMode === 'cruiser') {
                // Tense/ominous: slower, darker, with tremolo
                osc1.type = 'sawtooth';
                ...
                lfoGain1.gain.value = 140;
                gain1.gain.setValueAtTime(0.12, t);

                osc2.type = 'square';
                osc2.frequency.setValueAtTime(21, t);
                gain2.gain.setValueAtTime(0.06, t);
                lfo2.type = 'sine';
                lfo2.frequency.value = 0.35;
                lfoGain2.gain.value = 0.05;
            } else {
                // Default dark sci-fi drone
                osc1.type = 'sawtooth';
            ...
            osc1.start(); lfo1.start(); osc2.start(); lfo2.start();
        ...
        // Base zoom for the game (set to the previous Cruiser-fight zoom).
        ...
        class Spaceship extends Entity {
            constructor() {
                ...
                this.outerShieldRadius = this.shieldRadius + 26;
            ...
            draw(ctx) {
                ...
                if (this.invincibilityCycle.unlocked && this.invincibilityCycle.state === 'active') {
                    ...
                    ctx.beginPath();
                    const outerR = (this.outerShieldSegments && this.outerShieldSegments.some(s => s > 0)) ? this.outerShieldRadius : 0;
                    ctx.arc(0, 0, Math.max(this.shieldRadius, outerR) + 14, 0, Math.PI * 2);
                    ctx.stroke();
                    ctx.restore();
                }
                
                // Outer shield ring (purple, rotates opposite the main shield)
                if (this.outerShieldSegments && this.outerShieldSegments.some(s => s > 0)) {
                    ctx.save();
                    ctx.rotate(this.outerShieldRotation);
                    ctx.lineWidth = 4;
                    ctx.shadowBlur = 10;
                    ctx.shadowColor = '#b0f';
                    ...
                    for (let i = 0; i < outerCount; i++) {
                        if (this.outerShieldSegments[i] > 0) {
                            ...
                            ctx.globalAlpha = 0.9;
                            ctx.beginPath();
                            ctx.arc(0, 0, this.outerShieldRadius, i * outerAngle + 0.08, (i + 1) * outerAngle - 0.08);
                            ctx.stroke();
                        }
                    }
                    ctx.shadowBlur = 0;
                ...
                // Turbo flame VFX (big red/yellow exhaust)
                if (this.turboBoost && this.turboBoost.activeFrames > 0) {
                    const flicker = 0.75 + Math.random() * 0.35;
                    const flameLen = 95 * s * flicker;
                    const flameWid = 22 * s * flicker;
                    const baseX = -25 * s;

                    const grad = ctx.createLinearGradient(baseX, 0, baseX - flameLen, 0);
                    grad.addColorStop(0, 'rgba(255,255,180,0.95)');
                    grad.addColorStop(0.25, 'rgba(255,220,0,0.95)');
                    ...
                    ctx.beginPath();
                    ctx.moveTo(baseX, 0);
                    ctx.lineTo(baseX - flameLen, -flameWid);
                    ctx.lineTo(baseX - flameLen * 0.88, 0);
                    ctx.lineTo(baseX - flameLen, flameWid);
                    ctx.closePath();
                    ctx.fill();
                    ...
                    ctx.lineTo(baseX - flameLen * 0.75, 0);
                    ctx.lineTo(baseX - flameLen * 0.55, flameWid * 0.35);
                    ctx.closePath();
                    ctx.fill();
                    ctx.restore();
        ...
        class FlagshipGuidedMissile extends Entity { 
            ...
            draw(ctx) {
                ...
                ctx.fillStyle = '#c33';
                ctx.beginPath();
                ctx.moveTo(-len/2 + 14, w/2);
                ctx.lineTo(-len/2 - 2, w/2 + 8);
                ctx.lineTo(-len/2 + 8, w/2 - 2);
                ctx.closePath();
                ctx.fill();
                ctx.beginPath();
                ctx.moveTo(-len/2 + 14, -w/2);
        ...
        class Enemy extends Entity {
            ...
            update() {
                ...
                if (!this.disableAutoFire && distToPlayer < gunboatRange && !player.dead && this.freezeTimer <= 0) {
                    this.shootTimer--;
            ...
            draw(ctx) {
                ...
                ctx.save();
                ctx.translate(this.pos.x, this.pos.y);
                if (this.modifier === 'stealth') {
                    const alpha = 0.4 + Math.abs(Math.sin(Date.now() * 0.003)) * 0.4;
                    ctx.globalAlpha = alpha;
                }
                if (this.shieldSegments.length > 0) {
                    ...
                    for(let i=0; i<segCount; i++) {
                        if (this.shieldSegments[i] > 0) {
                            ctx.strokeStyle = this.isCruiser ? '#8ff' : (this.isGunboat ? (this.gunboatLevel === 1 ? '#f55' : '#fa0') : '#f0f');
                            ctx.beginPath();
                ...
                if (this.innerShieldSegments && this.innerShieldSegments.length > 0) {
                    ...
                    for (let i=0; i<innerCount; i++) {
                        if (this.innerShieldSegments[i] > 0) {
                            ctx.strokeStyle = this.isCruiser ? '#8ff' : (this.isGunboat ? (this.gunboatLevel === 1 ? '#f88' : '#ff0') : '#8ff');
                            ctx.beginPath();
                            ctx.arc(0, 0, innerRadius, i * innerAngle + 0.05, (i+1) * innerAngle - 0.15);
                            ctx.stroke();
                        }
                    }
                    ctx.restore();
                ...
                if (this.isGunboat) {
                    // Gunboat / Boss hull styling
                    const s = this.gunboatScale || 1.4;
                    if (this.isCruiser) {
                        ...
                        ctx.fillStyle = this.gunboatLevel === 1 ? '#211' : '#321';
                        ctx.strokeStyle = this.gunboatLevel === 1 ? '#f55' : '#fb0';
                    }
                    ctx.lineWidth = 2;
                    // path is built below
                } else if (this.type === 'elite_roamer') {
                ...
                ctx.lineWidth = 2;
                ctx.shadowBlur = this.isCruiser ? 10 : 5;
                ctx.shadowColor = this.isCruiser ? '#8899aa' : '#f00';
                
                ctx.beginPath();
                if (this.isGunboat) {
                    const s = this.gunboatScale || 1.4;
                    if (this.isCruiser) {
                        // Battlestar-inspired: long armored hull + twin flight pods
                        const L = 28 * s;
                        const H = 7.5 * s;
                        const nose = 6 * s;
                        const tail = 7 * s;
                ...
                // Cruiser detailing (pods/lines/engines) after base hull fill
                if (this.isCruiser) {
                    const s = this.gunboatScale || 1.4;
                    const L = 28 * s;
                    const H = 7.5 * s;
                    ...
                    ctx.lineWidth = 2;
                    ctx.beginPath();
                    ctx.rect(podX, podY - podH, podL, podH * 2);
                    ctx.rect(podX, -podY - podH, podL, podH * 2);
                    ctx.fill();
                    ctx.stroke();
                    ...
                    ctx.strokeStyle = '#8aa0b3';
                    ctx.lineWidth = 1;
                    ctx.beginPath();
                    ctx.moveTo(-L + tail + 8*s, 0);
                    ctx.lineTo(L - nose - 10*s, 0);
                    ctx.moveTo(-L + tail + 8*s, H * 0.55);
                    ctx.lineTo(L - nose - 14*s, H * 0.55);
                    ctx.moveTo(-L + tail + 8*s, -H * 0.55);
                    ctx.lineTo(L - nose - 14*s, -H * 0.55);
                    ctx.stroke();
                    ctx.globalAlpha = 1;
                    ...
                    ctx.shadowColor = '#0ff';
                    ctx.beginPath();
                    ctx.arc(-L + 5*s, 0, 2.8*s, 0, Math.PI*2);
                    ctx.fill();
                    ctx.shadowBlur = 0;
                }
                ...
                if (this.isGunboat) {
                     if (this.isCruiser) {
                         ...
                         ctx.beginPath(); ctx.rect(0, -5, 28, 10); ctx.fillStyle = '#0ff'; ctx.fill(); ctx.stroke();
                     }
                } else if (this.type === 'elite_roamer' || this.type === 'hunter') {
        ...
        class Base extends Entity {
            ...
            draw(ctx) {
                ...
                if (this.isCruiser) {
                    ...
                    ctx.strokeStyle = '#aaf';
                    ctx.beginPath();
                    rr(bodyLen * 0.38, -bodyWid * 0.38, bodyLen * 0.25, bodyWid * 0.76, 8 * hs);
                    ctx.fill();
                    ctx.stroke();
                    
                    // Two engine pods
                    const podX = -bodyLen * 0.52;
                    const podY = bodyWid * 0.55;
                    const podLen = 18 * hs;
                    const podWid = 8 * hs;
                    const pods = [
                        { x: podX, y: -podY },
                    ...
                    for (const p of pods) {
                        ...
                        ctx.beginPath();
                        rr(-podLen * 0.5, -podWid * 0.5, podLen, podWid, 6 * hs);
                        ctx.fill();
                        ctx.stroke();
                        ...
                        ctx.strokeStyle = '#888';
                        ctx.beginPath();
                        rr(-podLen * 0.68, -podWid * 0.25, podLen * 0.25, podWid * 0.5, 4 * hs);
                        ctx.fill();
                        ctx.stroke();
                        ...
                        if (moving) {
                            const flicker = 0.75 + Math.random() * 0.35;
                            const flameLen = (28 * hs) * flicker;
                            const flameWid = (10 * hs) * flicker;
                            const baseX = -podLen * 0.78;
                            ...
                            ctx.moveTo(baseX, 0);
                            ctx.lineTo(baseX - flameLen * 0.55, -flameWid * 0.35);
                            ctx.lineTo(baseX - flameLen * 0.72, 0);
                            ctx.lineTo(baseX - flameLen * 0.55, flameWid * 0.35);
                            ctx.closePath();
                            ctx.fill();
                        }
                    ...
                    ctx.save();
                    ctx.rotate(this.turretAngle);
                    ctx.fillStyle = '#331122'; ctx.strokeStyle = '#ffcc66';
                    ctx.beginPath();
                    ...
                    ctx.save();
                    ctx.rotate(this.angle);
                    ctx.fillStyle = '#112244'; ctx.strokeStyle = '#66ffff';
                    ctx.shadowBlur = 12; ctx.shadowColor = '#66ffff';

                    ctx.beginPath();
                    for(let i=0; i<5; i++) {
                        const a = i * Math.PI * 2 / 5;
                        ctx.lineTo(Math.cos(a)*65, Math.sin(a)*65);
                        ctx.lineTo(Math.cos(a+0.25)*20, Math.sin(a+0.25)*20);
                    }
                    ctx.closePath();
                    ctx.fill(); ctx.stroke();
                    ctx.restore();
                    ...
                    ctx.save();
                    ctx.rotate(this.turretAngle);
                    ctx.fillStyle = '#113355'; ctx.strokeStyle = '#66ffff';
                    ctx.beginPath();
                    ...
                    // Standard retro ringed station
                    ctx.save();
                    ctx.rotate(this.angle);
                    ctx.fillStyle = '#1a0f2d'; ctx.strokeStyle = '#ff77ff';
                    ctx.shadowBlur = 12; ctx.shadowColor = '#ff77ff';
                    ctx.lineWidth = 3;
        ...
        class SpaceNugget extends Entity {
	        ...
	        class CaveWallTurret extends Entity {  
	            ...
	            draw(ctx) { 
	                ...
	                if (this.armored && (this.armorHp > 0 || this.weakpointHp > 0)) { 
	                    ...
	                    ctx.beginPath(); 
	                    ctx.moveTo(0, 0); 
	                    ctx.lineTo(wp.x - this.pos.x, wp.y - this.pos.y); 
	                    ctx.stroke(); 
	                    ctx.fillStyle = '#0ff'; 
	                    ctx.beginPath(); 
	                    ctx.arc(wp.x - this.pos.x, wp.y - this.pos.y, 6 / z, 0, Math.PI * 2); 
	                    ctx.fill(); 
	                    ctx.restore(); 
	                } 
	                ...
	                if (this.mode === 'tracker' && (this.trackerLock > 0 || this.trackerBurst > 0)) { 
	                    ctx.save(); 
	                    ctx.rotate(this.trackerAngle - aim); 
	                    ctx.globalAlpha = 0.35; 
	                    ctx.strokeStyle = '#0ff'; 
	                    ctx.lineWidth = 2 / z; 
	        ...
	        class CaveWallSwitch extends Entity { 
	            ...
	            draw(ctx) { 
	                ...
	                ctx.shadowBlur = 18; 
	                ctx.shadowColor = '#0ff'; 
	                ctx.fillStyle = `rgba(0,255,255,${0.25 + pulse})`; 
	                ctx.strokeStyle = '#0ff'; 
	                ctx.lineWidth = 2 / z; 
	                ctx.beginPath(); 
	                ctx.arc(0, 0, this.radius, 0, Math.PI * 2); 
	        ...
	        class CaveRewardPickup extends Entity { 
	            ...
	            draw(ctx) { 
	                ...
	                ctx.lineWidth = 4 / z; 
	                ctx.shadowBlur = 18; 
	                const color = this.rewardType === 'upgrade'
	                    ? '#f0f'
	                    : (this.rewardType === 'shield'
	                        ? '#0ff'
	                        : (this.rewardType === 'fragment'
	                            ? '#ff0'
	                            : (this.rewardType === 'nugs' ? '#fa0' : '#0f0'))); 
	                ...
	                ctx.textAlign = 'center'; 
	                ctx.textBaseline = 'middle'; 
	                const label = this.rewardType === 'upgrade'
	                    ? 'UP'
	                    : (this.rewardType === 'shield'
	                        ? 'SH'
	                        : (this.rewardType === 'fragment'
	                            ? 'KF'
	                            : (this.rewardType === 'nugs' ? 'NG' : '$'))); 
	        ...
	        class CaveDraftZone extends Entity { 
	            ...
	            draw(ctx) { 
	                ...
	                ctx.save(); 
	                ctx.translate(this.pos.x, this.pos.y); 
	                // Fill the zone so it stands out from the background grid.
	                const aFill = 0.08 + pulse * 0.06;
	                const grad = ctx.createLinearGradient(0, -this.h/2, 0, this.h/2);
	                grad.addColorStop(0, `rgba(0, 255, 255, ${aFill * 0.55})`);
	                grad.addColorStop(1, `rgba(0, 120, 255, ${aFill})`);
	                ctx.globalAlpha = 1; 
	                ctx.fillStyle = grad;
	                ctx.fillRect(-this.w/2, -this.h/2, this.w, this.h);

	                ctx.globalAlpha = 0.55; 
	                ctx.strokeStyle = `rgba(0,255,255,${0.10 + pulse * 0.12})`; 
	                ctx.lineWidth = 2 / z; 
	                ctx.strokeRect(-this.w/2, -this.h/2, this.w, this.h); 

	                ctx.globalAlpha = 0.22; 
	                ctx.strokeStyle = `rgba(255,255,255,${0.10 + pulse * 0.10})`;
	                for (let i = -2; i <= 2; i++) { 
	        ...
	        class CaveLevel { 
	            ...
	            draw(ctx, camX, camY, height, zoom) {  
	                ...
	                const addBlockers = () => { 
	                    ...
	                    if (!this.exitUnlocked && this.exitSeal && this.exitSeal.segments && this.exitSeal.segments.length) {
	                        if (this.exitSeal.y >= y0 - 2600 && this.exitSeal.y <= y1 + 2600) {
	                            for (let j = 0; j < this.exitSeal.segments.length; j++) {
	                                const s = this.exitSeal.segments[j];
	                                if (!segFinite(s)) continue;
	                                ctx.moveTo(s.x0, s.y0);
	                                ctx.lineTo(s.x1, s.y1);
	                            }
	                    ...
	                    if (!this.exitUnlocked && this.exitSeal && this.exitSeal.sideSegments && this.exitSeal.sideSegments.length) {
	                        for (let j = 0; j < this.exitSeal.sideSegments.length; j++) {
	                            const s = this.exitSeal.sideSegments[j];
	                            if (!segFinite(s)) continue;
	                            if (s.y0 < y0 - 2600 || s.y0 > y1 + 2600) continue;
	                            ctx.moveTo(s.x0, s.y0);
	                            ctx.lineTo(s.x1, s.y1);
	                        }
	                    }
...
//space station class
        ...
        function completeContract(success = true) {
            ...
            if (contractId) {
                environmentAsteroids = environmentAsteroids.filter(a => a && a.contractId !== contractId);
                enemies = enemies.filter(e => e && e.contractId !== contractId);
            }
            activeContract = null;
        ...
        function scheduleNextRadiationStorm(fromNow = Date.now()) {
            // Keep them impactful but not constant.
            const min = 90000;   // 1.5m
            const max = 170000;  // 2.8m
            nextRadiationStormAt = fromNow + min + Math.floor(Math.random() * (max - min + 1));
        }

        function spawnRadiationStormRelative() {
            if (!player) return;
            const p = findSpawnPointRelative(true, 2400, 4200);
            const radius = 820 + Math.random() * 320;
            radiationStorm = new RadiationStorm(p.x, p.y, radius, 45000);
            showOverlayMessage("RADIATION STORM FORMED", '#ff0', 2000);
        }
        ...
        function resolveEntityCollision() {
            ...
            for (let entity of damageable) {
                ...
                const nearbyAsteroids = asteroidGrid.query(entity.pos.x, entity.pos.y);
	                ...
	                if (activeCave && (entity === player || entity instanceof Enemy || entity instanceof Base || (bossActive && boss && entity === boss))) {
        ...
        function applyUpgrade(id, tier) { 
            ...
            switch(id) { 
                ...
                case 'outer_shield':
                    ...
                    player.outerShieldRadius = player.shieldRadius + 26;
                    player.outerShieldSegments = new Array(player.maxOuterShieldSegments).fill(1);
                    break;
                case 'shield_regen':
        ...
        function mainLoop() {
            ...
            try {
                    ...
                    if (pick < 0.4) {
                        ...
                        activeContract = {
                         ...
                         activeContract = {
                             ...
                             coreCollected: false,
                             target: { x: target.x, y: target.y },
                             rewardNugs: 7 + Math.floor(Math.random() * 5),
                             rewardScore: 12000
                         };
                         const anomaly = new AnomalyZone(target.x, target.y, id);
                         contractEntities.anomalies.push(anomaly);
                         showOverlayMessage("NEW CONTRACT: INVESTIGATE ANOMALY", '#0f0', 2000, 2);
                         playSound('contract');
                     } else {
                         // Destroy Fortress
                         const id = `C${contractSequence}`;
	            ...
	            // Cruiser timed spawn: if timer active and no boss present, spawn a cruiser 
	             try { 
	                  if (!sectorTransitionActive && !warpActive && !caveMode && !inAnomaly && dreadManager.timerActive && !bossActive && dreadManager.timerAt && now >= dreadManager.timerAt) { 
            ...
            if (!caveMode) { 
	            ...
	            const caveActive = (caveMode && caveLevel && caveLevel.active);  
	            if (warpZone && warpZone.active) { warpZone.update(); warpZone.draw(ctx); }  
	            if (warpGate && !warpGate.dead) { warpGate.update(); warpGate.draw(ctx); }  
	            if (caveActive) { caveLevel.update(); }  
